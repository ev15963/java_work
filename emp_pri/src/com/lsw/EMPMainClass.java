...
public class EMPMainClass {
	public static void main(String[] args) {
		...
		// 국내에 근무하는 인원 수
		// code배열/ 인원수(int count=0)
		// 인원수 : code 배열의 값들 중 "KR"과 같은 지를 비교
		// 비교 : if(){ }
		...
		System.out.println("국내 근무자 수 : " + count);
		// 국내 근무자의 급여합
		...
		System.out.println("국내 근무자 급여 평균:" + avg);
		// System.out.println("국내 근무자 급여 평균:"+(sum/code.length));
		...
		//국내 근무자의 명단을 출력후
		//해당 명단을 이용하여 다른 일을 추가로 할 경우
		//출력 형식: LSD, LKO,
		//필요 자원 : code 배열 /name 배열
		//필요 구문 : for(){} / if(){}
		//필요 변수 : String sumName="";
		//합 구하기 : sumName=sumName+name[i]+",";
		...
		String content="경기남부경찰청 수사본부는 2일 기자회견을 열고 이춘재 사건 종합 수사결과를 발표했다. 이 자리에서 경찰은 이춘재가 14건의 살인과 9건의 강간 사건을 저지른 것으로 확인했다면서 이춘재가 (성적) 욕구를 풀기 위해 가학적 형태의 연쇄 범행을 한 것으로 확인했다고 밝혔다." + 
				"" + 
...
/*		System.out.println(content);
		...
		for(int i=0;i<words.length;i++) {
			if(words[i].contains("이춘재")) { //contains 이춘재를 포함하는
				cnt++;
			}
		}
